import { Customer, Employee, optionService, Pet, Service } from "@/types/models";
...
export default function CashierPage() {
  ...
  const [selectedStaffId, setSelectedStaffId] = useState<Employee | null>(null);
  ...
  const chooseCustomer = async (c: Customer) => {
    ...
    setPets([]);
    const res = await fetch(`/api/customers/${c.id}/pets`);
    const json = await res.json();
    setPets(json?.data ?? []);
  };
                            ...
                            <div className="mt-2 grid gap-2 md:grid-cols-2">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {svcMeta.options.map((opt) => {
                                  const active = svc.options.includes(opt.id);
                                  return (
                                    <button
                                      key={opt.id}
                                      type="button"
                                      onClick={() => toggleOption(card.key, svcIdx, opt.id)}
